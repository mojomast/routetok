export function createStudioChat({ document, getStudio, request, buildMessages, parseResponse, acceptProposal, save, renderText, metricText, uid, now, onStatus }) {
  ...
  function render() {
    ...
    for (const message of chat.messages) {
      ...
      if (message.proposal && message.proposal.status === "pending") {
        ...
        accept.onclick = async () => {
          ...
          try { acceptProposal(message.proposal); message.proposal.status = "accepted"; await save(); render(); }
          catch (error) { onStatus(error.message); }
        };
        reject.onclick = async () => { message.proposal.status = "rejected"; await save(); render(); };
    ...
    $("studio-chat-draft").disabled = Boolean(controller);
    $("send-studio-chat").disabled = Boolean(controller);
    $("stop-studio-chat").hidden = !controller;
  }

  async function send(frozenContext = []) {
    if (controller) return onStatus("Wait for the active steering response or stop it");
    const studio = getStudio(); const chat = normalize(); const input = $("studio-chat-draft"); const content = input.value.trim();
    if (!content) return;
    ...
    controller = new AbortController(); chat.messages.push(user); chat.draft = ""; input.value = ""; render(); await save();
    try {
      const result = await request(agent, buildMessages(studio, agent, content, frozenContext), controller.signal);
    ...
    controller = null; chat.messages = chat.messages.slice(-80); await save(); render();
  }

  function stop() { controller?.abort(); }

  $("studio-chat-draft")?.addEventListener("input", (event) => { normalize().draft = event.target.value; save(); });
