...
const server = createServer(
  ...
  try {
    ...
    if (pathname.startsWith("/admin/api/")) {
      ...
      if (request.method === "GET" && pathname === "/admin/api/evaluation") {
        try {const {evaluationSummary}=await import('./evaluation-report.js');json(response,200,evaluationSummary(process.env.ROUTETOK_EVALUATION_REPORT));}
        catch {json(response,200,{available:false,error:'invalid_or_unavailable_report'});}return;
      }
