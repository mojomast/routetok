...
For AgentRouter OpenAI Chat Completions targeting upstream `deepseek-v4-flash` or `deepseek-v4-pro` (including hyphen-suffixed variants), nonempty tools plus any assistant history with missing/non-string `reasoning_content` cause RouteTok to add `thinking: { "type": "disabled" }`, but only when neither `thinking` nor `reasoning_effort` is explicitly present. This avoids v4's default-thinking replay requirement at the cost of non-thinking inference for that attempt. Supplied reasoning (including empty strings), messages, and tools are preserved; unrelated fallback attempts are unchanged. Explicit thinking requests still require complete client replay. See [troubleshooting](troubleshooting.md#deepseek-v4-tool-requests-return-400).

For AgentRouter OpenAI Chat Completions targeting the `deepseek-v4-*` family, RouteTok translates `response_format: { "type": "json_schema", "json_schema": { "schema": … } }` — which the upstream adapter rejects with `400 This response_format type is unavailable now` — into a single appended function tool carrying the caller's schema with `tool_choice: "auto"`, leaving the caller's thinking setting intact. The returned tool call is unwrapped back into `message.content` (and, for streams, `delta.content`) with `finish_reason: "stop"`, so clients keep the native content-JSON contract. Because the adapter rejects a forced `tool_choice` while thinking mode is active, a non-stream auto attempt that returns no matching tool call and no usable JSON content — including a `finish_reason: "length"` answer whose budget was consumed by reasoning — is retried once with a forced tool and `thinking: { "type": "disabled" }`. A committed stream cannot be retried once reasoning or text has been forwarded. Boolean `thinking: false`, `enable_thinking: false`, and `chat_template_kwargs.enable_thinking: false` are normalized to the accepted `thinking: { "type": "disabled" }` shape, while a supplied `reasoning_effort: "none"` is preserved as the canonical thinking-off switch. All adjustments are per attempt and scoped to this provider/protocol/endpoint/model family; other models, providers, endpoints, and formats are unchanged. Thinking-on `json_schema` calls need enough output budget for the model's reasoning before it calls the schema tool.
...
An opened circuit keeps a model out of candidate chains until `circuitOpenMs` elapses. An expired circuit transitions to half-open and admits at most one in-flight probe at a time (no concurrent probes). Dispatch atomically rechecks circuit, cooldown, and entitlement state so stale candidate lists cannot bypass admission; skipped routes do not count as upstream attempts. A failed, rate-limited, or entitlement-blocked probe re-opens the circuit immediately, and opening resets the inherited failure streak so the single probe outcome governs the next circuit state; a successful probe closes the circuit and resets model health.
