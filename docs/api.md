...
For AgentRouter OpenAI Chat Completions targeting the `deepseek-v4-*` family, RouteTok translates `response_format: { "type": "json_schema", "json_schema": { "schema": … } }` — which the upstream adapter rejects with `400 This response_format type is unavailable now` — into a single forced function tool carrying the caller's schema, with thinking disabled. The upstream tool call is unwrapped back into `message.content` (and, for streams, `delta.content`) with `finish_reason: "stop"`, so clients keep the native content-JSON contract. Boolean `thinking: false`, `enable_thinking: false`, and `chat_template_kwargs.enable_thinking: false` are normalized to the accepted `thinking: { "type": "disabled" }` shape, while a supplied `reasoning_effort: "none"` is preserved as the canonical thinking-off switch. All adjustments are per attempt and scoped to this provider/protocol/endpoint/model family; other models, providers, endpoints, and formats are unchanged. Because the upstream adapter rejects any forced `tool_choice` while thinking mode is active, clients that force a function tool without disabling thinking still receive the upstream `400`.
