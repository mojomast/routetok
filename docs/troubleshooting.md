...
## DeepSeek v4 tool requests return 400

[DeepSeek's thinking-mode guide](https://api-docs.deepseek.com/guides/thinking_mode) describes thinking as enabled by default for v4. When sending tools, clients must replay the full `reasoning_content` on every historical assistant message, including plain answers without `tool_calls`. Missing reasoning can cause an upstream `400`; a status alone does not establish that this was the cause.

For AgentRouter OpenAI Chat Completions attempts only, RouteTok adds `thinking: { "type": "disabled" }` when the canonical upstream model is `deepseek-v4-flash` or `deepseek-v4-pro` (including hyphen-suffixed variants), `tools` is a nonempty array, and any historical assistant message has missing or non-string `reasoning_content`. This applies only when neither `thinking` nor `reasoning_effort` is explicitly present. An empty reasoning string counts as supplied. Complete replay, initial requests without assistant history, requests without tools, other models/providers, and other endpoints are unchanged. The adjustment is per attempt, so it does not carry over to an unrelated fallback model.

The tradeoff is non-thinking inference for that attempt rather than a likely replay-validation failure. RouteTok preserves the original messages, tool calls, tool results, tool definitions, and any supplied reasoning; it does not fabricate reasoning or flatten OpenAI tool history. Explicit mode settings are passed through, even if incomplete replay may still produce a `400`.

To keep thinking enabled, fix the client to accumulate streamed `delta.reasoning_content` chunks for each assistant response and store/replay the complete string on that assistant message in subsequent requests, including plain answers. For non-stream responses, retain `message.reasoning_content` alongside content and tool calls. Do not substitute invented reasoning for missing history; restart with complete history or explicitly choose `thinking: { "type": "disabled" }`.
...
Because the adapter rejects a forced `tool_choice` while thinking mode is active, the auto path is preferred. If a non-stream auto attempt produces no matching tool call and no usable JSON content — including a `finish_reason: "length"` answer where reasoning consumed the whole budget — RouteTok retries that attempt once with a forced tool and `thinking: { "type": "disabled" }` to guarantee the schema. A committed stream cannot be retried once reasoning or text has been forwarded. Thinking-on structured calls therefore need a sufficient `max_tokens`; raise the budget rather than relying on the fallback.
